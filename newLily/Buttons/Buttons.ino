...
void setup()
{
  ...
  pinMode(AT_LED, OUTPUT);
  pinMode(butBlack, INPUT);
...
void loop()
{
  ...
  if (readBut(butBlack)) {
    ...
    if (SerialAT.available()) {
      String r = SerialAT.readString();
      if (r.indexOf("CSCLK: 1") >= 0) {
        digitalWrite(LED_PIN, HIGH);
        delay(50);
        digitalWrite(LED_PIN, LOW);
      }
      else if (r.indexOf("CSCLK: 0") >= 0) {
        digitalWrite(LED_PIN, LOW);
        delay(50);
        digitalWrite(LED_PIN, HIGH);
      }
