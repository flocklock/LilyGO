...
void loop() {
  ...
  if (voltage < 3.3 && voltage > 1.0) {
    for (int i = 0; i < 10; i++) {
      ...
      if (voltage > 3.3)
        break;
      file = SD.open("/acc.txt", FILE_APPEND);
      file.print("low battery\n");
      file.close();
      delay(1000);
      esp_sleep_enable_timer_wakeup(86400 * uS_TO_S_FACTOR);
  ...
  for (int i = 0; i < 31; i++) {
    ...
    message.append(std::to_string(millis()) + "," + std::to_string(readBattery(BAT_ADC)).substr(0,4) + "," + std::to_string(e.X).substr(0,5) + "," + std::to_string(e.Y).substr(0,5) + "," + std::to_string(e.Z).substr(0,5) + "\n");
  }
  if (message.size() >= 39000) {
