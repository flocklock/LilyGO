...
void sendAcc()
{
  ...
  float acc = (fabs(event.acceleration.x) + fabs(event.acceleration.y), + fabs(event.acceleration.z));
...
void setup()
{
  ...
  timer.setInterval(5000L, sendAcc);
  timer.setInterval(10000L, sendLocation);
}
